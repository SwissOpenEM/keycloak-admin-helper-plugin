...
public class RealmsAdminResourceOverride extends RealmsAdminResource {
    public RealmsAdminResourceOverride(KeycloakSession session, AdminAuth auth, TokenManager tokenManager) {
        super(session, auth, tokenManager);
    }
//    protected static final Logger logger = Logger.getLogger(RealmsAdminResourceOverride.class);
...
//    }
    ...
    public RealmAdminResource getRealmAdmin(@PathParam("realm") @Parameter(description = "realm name (not id!)") final String name) {
        ...
        return new RealmAdminResourceOverride(session, realmAuth, adminEvent);
    }
