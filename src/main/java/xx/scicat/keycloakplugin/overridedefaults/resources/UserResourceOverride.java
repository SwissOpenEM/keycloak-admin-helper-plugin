...
public class UserResourceOverride extends UserResource {
    private static final Logger logger = Logger.getLogger(UserResourceOverride.class);
    ...
    public UserResourceOverride(KeycloakSession session, UserModel user, AdminPermissionEvaluator auth, AdminEventBuilder adminEvent) {
    ...
    public Response updateUser(final UserRepresentation rep) {
        ...
        try {
            ...
            if (rep.getAttributes() == null) {
                ...
                for (Entry<String, List<String>> entry : user.getAttributes().entrySet()) {
            ...
            session.getTransactionManager().setRollbackOnly();
            Properties messages = org.keycloak.services.resources.admin.AdminRoot.getMessages(session, realm, auth.adminAuth().getToken().getLocale());
